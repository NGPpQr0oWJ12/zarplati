...
В интерактивном CLI-режиме скрипт задает вопросы на русском языке. На выборе режима можно просто нажать Enter: будет выбран `1) deploy`. По умолчанию production-сервис слушает `0.0.0.0`, поэтому приложение доступно из локальной сети по адресу сервера, например `http://192.168.134.16:<порт>`.
